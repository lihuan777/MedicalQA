...
from LoadDataset import getAnswers,getCategories
import json
import os
...
def add_categories(originPath,categoriesPath,savePath):
    ...
    with open(f"{savePath}",'w',encoding='utf-8') as f:
        ...
        for i in range(dataSize):
            ...
            # 当前为后置类型医生字段
            originData[i]['ques_title'] = originData[i]['ques_title'] + categories
...
def DPR_add_categories_candidate(originPath,savePath):
    ...
    with open(f"{savePath}", "w", encoding="utf-8", newline="") as f:
        ...
        for i in range(dataSize):
            ...
            # 当前为后置类型医生字段
            query = line['ques_title'].replace(' ','').replace('"','').replace("'","").replace("＼","").replace('\\','') + categories
...
if __name__ == '__main__':
    ...
    # 处理原始测试集
    originPath = "/data/lihuan/MedicalQA/1_translate/cn_test.json"
    categoriesPath = "/data/lihuan/MedicalQA/old/translate/loc14_test.json"
    savePath = "/data/lihuan/MedicalQA/1_translate/ES检索_1Q3A_加医生类型（后置类型）/cn_test_categories.json"
    add_categories(originPath,categoriesPath,savePath)
    
    # 处理原始训练集15W数据1Q3A
    originPath = "/data/lihuan/MedicalQA/1_translate/cn_train_15W_3.json"
    categoriesPath = "/data/lihuan/MedicalQA/old/translate/loc14_train.json"
    savePath = "/data/lihuan/MedicalQA/1_translate/ES检索_1Q3A_加医生类型（后置类型）/cn_train_15W_categories.json"
    add_categories(originPath,categoriesPath,savePath)
    
    # 处理DPR生成的需要进行BART的训练集15W数据1Q3A
    originPath = "/data/lihuan/DPR/DPR-main/outputs/translate_BART_1Q3A_train.json"
    categoriesPath = "/data/lihuan/MedicalQA/old/translate/loc14_train.json"
    savePath = "/data/lihuan/MedicalQA/1_translate/DPR检索_1Q3A_加医生类型（后置类型）/DPR_add_categories_train.json"
    add_categories(originPath,categoriesPath,savePath)
