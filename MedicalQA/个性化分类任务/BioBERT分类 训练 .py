...
def run_train(dataset):
    ...
    # 加载BERT Tokenizer
    tokenizer = BertTokenizer.from_pretrained('个性化分类任务/模型/biobert-base-cased-v1.1')
    ...
    # 加载BERT模型
    model = BertForSequenceClassification.from_pretrained('个性化分类任务/模型/biobert-base-cased-v1.1', num_labels=len(label_encoder.classes_))
    model.to(device)  # 将模型移到device
    # ！！！！！！！！！！！！如果跑不起来使用其他通用模型Auto加载函数使用模型
    ...
    training_args = TrainingArguments(
        output_dir='./' + dataset + '_BioBERT',          # 输出目录
