...
{{
  ...
  "reason": "修正理由（如无需修正请注明'维持原分类'）"
    ...
    # 置信度动态调整部分
    def _get_dynamic_threshold(self, text_length):
        """根据文本长度动态调整置信度阈值"""
        adjustment = min(text_length / 500 * 0.1, 0.15)  # 长文本最多降低0.15阈值
        return self.confidence_threshold - adjustment
    ...
    def predict_pipeline(self, test_file, output_dir):
        ...
        for text in tqdm(test_df['text'].tolist(), desc="Processing"):
            ...
            current_threshold = self._get_dynamic_threshold(text_length)
            
            # 高置信度直接通过（修改判断条件）
            if bert_result["confidence"] >= current_threshold:
...
if __name__ == "__main__":
    # 配置参数
    bert_model_path = "个性化分类任务/结果/BERT/checkpoint"
    chatglm_model_path = "个性化分类任务/模型/glm-4-9b-chat"
    test_file = "个性化分类任务/数据集/Huatuo-26M/test_data_cleaned.json"
    output_dir = "个性化分类任务/结果/BERT+ChatGLM"
