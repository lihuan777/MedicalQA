...
def run_train(dataset):
    ...
    # 加载 RoBERTa 模型并添加分类头
    model = RobertaForSequenceClassification.from_pretrained('hfl/chinese-roberta-wwm-ext', num_labels=len(label_encoder.classes_))
    model.to(device)  # 将模型移到device

    # 定义训练参数
    training_args = TrainingArguments(
        output_dir='./' + dataset + '_RoBERTa',          # 输出目录
