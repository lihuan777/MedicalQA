...
def run_train(dataset):
    ...
    tokenizer = AutoTokenizer.from_pretrained('个性化分类任务/模型/ERNIE')
    ...
    model = AutoModelForSequenceClassification.from_pretrained('个性化分类任务/模型/ERNIE', num_labels=len(label_encoder.classes_))
    model.to(device)  # 将模型移到device

    # 定义训练参数
    training_args = TrainingArguments(
        output_dir='./' + 'checkpoint-'  + dataset + '-ERNIE',          # 输出目录
